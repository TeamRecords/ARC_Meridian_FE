# ARC_Meridian_FE


## 📜 License & Usage

This project includes both open-source and proprietary components.

- **/FreeAssets** — Licensed under the [MIT License](./LICENSE).  
  You’re free to use, modify, and redistribute these files under standard MIT terms.

- **/MeridianDev** — © 2025 TeamRecords. All rights reserved.  
  This directory contains proprietary and trademarked materials.  
  Usage, redistribution, or modification without explicit written permission is prohibited.  
  See the [NOTICE](./NOTICE) file for full details.

For commercial licensing or trademark inquiries, contact:  
**A.R.C Foundation Licensing Department**  
📧 *[Official Website](https://www.arcfoundation.net/profile/cmhkdi4vy0000vsy0inw3fgko)*
